...
#setting up an https context to fetch the data from openml
...
while(True):
    ...
    try:
        ...
        #drawing a box in center of the video
        height,width = gray.shape
        upper_left = (int(width/2-56),int(height/2-56))
        bottom_right = (int(width/2+56),int(height/2+56))
        cv2.rectangle(gray,upper_left,bottom_right,(0,255,0),2)

        #to consider the area inside the box for ditecting the digit
        #roi:region of nterest
        roi = gray[upper_left[1]:bottom_right[1],upper_left[0]:bottom_right[0]]

        #converting cv2 image to pil format
        im_pil = Image.fromarray(roi)
        
        #convert to grayscale image 'L' format which means that the pixel is represented in a single value from 0 to 255
